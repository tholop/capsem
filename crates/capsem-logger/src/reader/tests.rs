...
fn opening_a_reader_serves_security_ledgers_from_disk_without_mirroring_them() {
    ...
    // Control: a hot ledger is mirrored, so `mem` really is attached and
    // populated. Without this, the assertions below would pass on a reader
    // that simply failed to build any memory schema at all.
